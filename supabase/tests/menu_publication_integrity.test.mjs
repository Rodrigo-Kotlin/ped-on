...
async function saveUnitOperationalConfig(client, unitId, config) {
  const r = await client.query(`select * from public.save_unit_operational_config($1, $2) as out`, [
    unitId,
...
async function setUnitActive(client, unitId, isActive) {
  const r = await client.query(`select * from public.set_unit_active($1, $2) as out`, [
    unitId,
    isActive,
  ]);
  return r.rows[0].out;
...
async function publish(client, unitId) {
  const r = await client.query(`select * from public.publish_unit_menu($1) as out`, [unitId]);
  return r.rows[0].out;
}

async function getPublicMenu(client, slug) {
  const r = await client.query(`select * from public.get_public_menu($1) as out`, [slug]);
  return r.rows[0].out;
}

async function getUnitPublication(client, unitId) {
  const r = await client.query(`select * from public.get_unit_menu_publication_admin($1) as out`, [
    unitId,
  ]);
  return r.rows[0].out;
}
...
async function run() {
  ...
  try {
    ...
    const managerOther = await createTestUser(
      admin,
      `pub-manager-other-${suffix}@pedon-test.invalid`,
    );
    ...
    orgA = (await ownerAS.query(`select public.complete_onboarding('Lanches do Zé') as org`))
      .rows[0].org;
    ...
    unitA2 = (await ownerAS.query('select (public.create_unit($1)).id as id', ['Restaurante A2']))
      .rows[0].id;

    orgB = (await ownerBS.query(`select public.complete_onboarding('Org B') as org`)).rows[0].org;
    ...
    const xbacon = await createProduct(
      ownerAS,
      unitA1,
      lanches.id,
      'X-Bacon',
      'Pão, carne, bacon e queijo',
      '29.90',
    );
    const xbaconInativo = await createProduct(
      ownerAS,
      unitA1,
      lanches.id,
      'X-Bacon Inativo',
    ...
    const xbaconIndisponivel = await createProduct(
      ownerAS,
      unitA1,
      lanches.id,
      'X-Bacon Indisponível',
      null,
      '31.00',
    );
    await setProductAvailable(ownerAS, xbaconIndisponivel.id, false);
    const refrigerante = await createProduct(
      ownerAS,
      unitA1,
      bebidas.id,
      'Refrigerante',
    ...
    ok(
      r1.version_number === 1,
      `84.1 primeira publicação gera version_number 1 (obtido ${r1.version_number})`,
    );
    ...
    const catsInSnap = new Set(
      snapRows.map((r) => r.cat).filter((c) => c !== null && c !== undefined),
    );
    ...
    ok(
      snapRows.some((r) => r.prod === 'X-Bacon'),
      '85.5 X-Bacon presente',
    );
    ok(
      snapRows.some((r) => r.prod === 'X-Bacon Indisponível'),
      '85.6 produto indisponível permanece no snapshot',
    );
    ok(
      snapRows.some((r) => r.prod === 'Refrigerante'),
      '85.7 Refrigerante presente',
    );
    ...
    ok(
      !snapRows.some((r) => r.prod === 'Escondido'),
      '85.10 produto de categoria inativa excluído',
    );
    ...
    ok(
      isUuid(xbaconSnap.source_product_id) && isUuid(xbaconSnap.source_category_id),
      '85.16 rastreabilidade source_* preservada',
    );
    ...
    ok(
      currentRow.rows[0].current_menu_version_id === r3.version_id,
      '86.5 ponte aponta para a versão corrente',
    );
    ...
    await expectError(
      anon,
      'select * from public.menu_versions limit 1',
      [],
      '42501',
    ...
    await expectError(
      anon,
      'select * from public.menu_version_categories limit 1',
      [],
      '42501',
    ...
    await expectError(
      anon,
      'select * from public.menu_version_products limit 1',
      [],
      '42501',
    ...
    await expectError(
      anon,
      'select * from public.menu_publications limit 1',
      [],
      '42501',
    ...
    assertTextMoney(
      '88.2 preço publicado (X-Bacon)',
      productByName(menuCategories(pub), 'X-Bacon').price,
      '29.90',
    );
    ...
    assertTextMoney(
      '88.3 mudança de preço não vaza ao snapshot',
      productByName(menuCategories(pub), 'X-Bacon').price,
      '29.90',
    );
    ...
    pub = await getPublicMenu(anon, currentSlug);
    ok(
      productByName(menuCategories(pub), 'X-Bacon') !== null,
      '88.4 mudança de nome não vaza ao snapshot',
    );
    ok(
      productByName(menuCategories(pub), 'Super Bacon') === null,
      '88.5 nome novo ausente no menu público',
    );
    ...
    ok(
      xbaconPub.description === 'Pão, carne, bacon e queijo',
      '88.6 mudança de descrição não vaza ao snapshot',
    );
    ...
    pub = await getPublicMenu(anon, currentSlug);
    ok(
      productInCategory(menuCategories(pub), 'Lanches', 'X-Bacon') !== null,
      '88.7 mudança de categoria não vaza ao snapshot',
    );
    ok(
      productInCategory(menuCategories(pub), 'Ofertas', 'X-Bacon') === null,
      '88.8 categoria de destino ainda ausente',
    );

    await createProduct(ownerAS, unitA1, lanches.id, 'Batata', null, '10.00');
    pub = await getPublicMenu(anon, currentSlug);
    ok(
      productByName(menuCategories(pub), 'Batata') === null,
      '88.9 produto novo não entra no snapshot corrente',
    );
    ...
    ok(
      productByName(menuCategories(pub), 'X-Bacon') !== null,
      '88.10 desativação não remove item do snapshot corrente',
    );
    ...
    ok(
      productByName(menuCategories(pub), 'Batata') !== null,
      '89.3 produto novo entra na nova versão',
    );
    ok(
      productByName(menuCategories(pub), 'X-Bacon') === null,
      '89.4 produto inativo sai na nova versão',
    );
    ok(productByName(menuCategories(pub), 'Super Bacon') === null, '89.5 nome antigo não aparece');
    ok(
      productByName(menuCategories(pub), 'X-Bacon Indisponível') !== null,
      '89.6 indisponível permanece na nova versão',
    );
    ...
    ok(
      v3products.rows[0].description === 'Pão, carne, bacon e queijo',
      '89.11 descrição original preservada na v3',
    );

    scenario(90, 'API pública');
    ok(pub.found === true, '90.1 menu público acessível sem sessão');
    ok(
      pub.organization && pub.organization.name === 'Lanches do Zé',
      '90.2 organization.name exposto',
    );
    ...
    ok(
      typeof refrigerantePub.price === 'string' && /^[0-9]+\.[0-9]{2}$/.test(refrigerantePub.price),
    ...
    pub = await getPublicMenu(anon, currentSlug);
    ok(
      productByName(menuCategories(pub), 'Refrigerante').is_available === false,
      '91.2 indisponibilizar reflete imediatamente',
    );
    ok(pub.menu.version_number === 4, '91.3 sem nova versão gerada');
    await setProductAvailable(operatorAS, refrigerante.id, true);
    pub = await getPublicMenu(anon, currentSlug);
    ok(
      productByName(menuCategories(pub), 'Refrigerante').is_available === true,
      '91.4 disponibilizar de volta reflete',
    );
    ...
    pub = await getPublicMenu(anon, currentSlug);
    ok(
      productByName(menuCategories(pub), 'Fantasma').is_available === false,
      '92.3 produto sem fonte fica is_available=false',
    );
    ...
    assertTextMoney(
      '93.4 minimum_order_amount como string',
      pub.operation.minimum_order_amount,
      '20.00',
    );
    ...
    ok(
      sunday.is_open === false && sunday.open_time === null,
      '93.6 dia fechado com horários nulos',
    );
    const monday = pub.operation.business_hours.find((h) => h.weekday === 1);
    ok(
      monday.is_open === true && monday.open_time === '09:00' && monday.close_time === '22:00',
      '93.7 dia aberto com horários',
    );
    const pix = pub.operation.payment_methods.find((p) => p.method === 'pix');
    const cash = pub.operation.payment_methods.find((p) => p.method === 'cash');
    ok(
      pix && pix.is_enabled === true && cash && cash.is_enabled === true,
      '93.8 métodos de pagamento refletidos',
    );
    ...
    ok(
      pubA2Inactive.operation.accepting_orders === false,
      '95.4 unidade inativa derruba accepting_orders',
    );
    await setUnitActive(ownerAS, unitA2, true);
    const pubA2Active = await getPublicMenu(anon, a2pub.public_slug);
    ok(
      pubA2Active.operation.accepting_orders === true,
      '95.5 reativação restaura accepting_orders',
    );

    await createCategory(ownerBS, unitB1, 'Cardápio B');
    const b1cat = (
      await ownerBS.query('select id from public.catalog_categories where unit_id = $1 limit 1', [
        unitB1,
      ])
    ).rows[0];
    ...
    ok(
      pubB1.operation.accepting_orders === false,
      '95.7 sem configuração -> accepting_orders=false',
    );
    ...
    const ownerForeignRead = await getUnitPublication(ownerAS, unitB1)
      ...
      .catch((e) => e);
    ok(
      ownerForeignRead && ownerForeignRead.code === 'PED11',
      '96.1 owner A não lê publicação de B1 (PED11)',
    );
    const adminViewForeign = await getUnitPublication(ownerBS, unitB1);
    ok(
      adminViewForeign.publication.exists === true &&
        adminViewForeign.unit.name === 'Unidade principal',
      '96.1b owner B lê sua própria publicação',
    );
    const ownerForeign = await publish(ownerAS, unitB1).catch((e) => ({ error: e }));
    ok(
      ownerForeign.error && ownerForeign.error.code === 'PED11',
      '96.2 owner A não publica unidade B (PED11)',
    );
    const adminViewA1 = await getUnitPublication(ownerAS, unitA1);
    ok(
      adminViewA1.publication.public_slug === currentSlug,
      '96.3 leitura administrativa A1 confere slug',
    );
    ok(
      adminViewA1.current_version.version_number === 5,
      '96.4 current_version aponta para versão corrente',
    );
    ok(
      Array.isArray(adminViewA1.history) && adminViewA1.history.length === 5,
      '96.5 histórico com 5 versões',
    );
    ...
    ok(
      c1.status === 'fulfilled' && c2.status === 'fulfilled',
      '97.1 ambas as publicações concluem',
    );
    ...
    ok(
      pair[0] === 6 && pair[1] === 7,
      `97.3 versões são sequenciais (6 e 7), obtido ${pair.join(', ')}`,
    );
    ...
    ok(
      productInCategory(menuCategories(pubA2Now), 'Cardápio A2', 'Prato A2') !== null,
      '98.2 A2 expõe seu produto',
    );
    ok(
      productByName(menuCategories(pubA2Now), 'Prato B') === null,
      '98.3 A2 não vaza produtos de outra unidade',
    );
  } finally {
