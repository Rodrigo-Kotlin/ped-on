...
| STATUS | IN_PROGRESS |
| CHECKPOINT | DB/ledger core completed |
...
| SUPABASE | CONNECTED — projeto `ped-on`; project ref `zmuxkztnilnzjyyojbbr`; região South America (São Paulo); migrations Local == Remote até `20260811080000` (12 migrations); migrations do Clube e hardening aplicadas oficialmente; `supabase db lint --linked` sem erros; Auth não alterado e zero e-mails enviados no Prompt 09; regras anteriores permanecem ativas |
...
| TESTES | Frontend unit/component 87/87; E2E total 104/104 em 360/768/1024/1440; banco: loyalty 108/108, pedidos 318/318, publicação 121/121, catálogo 123/123, operacional 80/80, RLS 22/22 e RBAC 31/31; `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm build`, Gitleaks e `supabase db lint --linked` PASS; migration list Local == Remote com 12 migrations |
| PWA | Nenhum cache privado novo; service worker continua sem `runtimeCaching` de API/dados/tokens |
| PENDÊNCIAS | Prompt 09: Edge Function `loyalty-cpf` (validação de CPF, HMAC-SHA-256 keyed, segredo `LOYALTY_CPF_HMAC_KEY` backend-only, `service_role` apenas na Edge, `Cache-Control: no-store`, CORS, smoke com CPF sintético); identidade do consumidor (token opaco 2h somente em memória, mismatch genérico); página pública `/clube/:publicSlug` (lookup, enroll, consentimento, saldo, extrato, checkout opcional com Clube); painel `/app/clube` owner-only (enable/disable, métricas, membros mascarados); frontend unit tests + E2E + 360/768/1024/1440 + a11y; build, gitleaks, CI e Edge deploy; heranças: ícones PWA definitivos, TypeScript 7.x, gestão de `membership_units` via UI, bundle JS ~734 kB, warning Node 20 em actions de terceiros |
| NEXT_STEP | Edge `loyalty-cpf` + identidade protegida por HMAC (continuação do Prompt 09) |
